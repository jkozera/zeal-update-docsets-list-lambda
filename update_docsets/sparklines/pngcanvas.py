...
class PNGCanvas:
    ...
    def dump(self):
        ...
        for y in range(self.height):
            raw_list.append(b'\x00')  # filter type 0 (None)
            for x in range(self.width):
                raw_list.append(struct.pack("!3B", *self.canvas[y][x]))
        raw_data = b''.join(raw_list)

        # 8-bit image represented as RGB tuples
        # simple transparency, alpha is pure white
        return signature + \
          self.pack_chunk(b'IHDR', struct.pack("!2I5B", self.width, self.height,
                                               8, 2, 0, 0, 0)) + \
          self.pack_chunk(b'tRNS', struct.pack("!6B",
                                               0, 0xFF, 0, 0xFF, 0, 0xFF)) + \
          self.pack_chunk(b'IDAT', zlib.compress(raw_data, 9)) + \
